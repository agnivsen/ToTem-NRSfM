...
%    C = colorcube(num_points+50);
    for ii = 1 : num_pointClouds
        ...
        if (size(Points, 1) == 2)
            scatter(Points(1, vis), Points(2, vis), 2,  C(vis, :), 'filled', 'Marker', 'o');
        end
        if (size(Points, 1) == 3)
            scatter3(Points(1, vis), Points(2, vis), Points(3, vis), 2, C(vis, :), 'filled', 'Marker', 'o');
        end
    ...
    for ii = 1 : num_pointClouds
        ...
        vis = logical(VisVecCell{ii});
        if (size(Points, 1) == 2)
            scatter(Points(1, vis), Points(2, vis), 8,  color_point_cloud, 'Marker', 'x');
        end
        if (size(Points, 1) == 3)
            scatter3(Points(1, vis), Points(2, vis), Points(3, vis), 8, color_point_cloud, 'Marker', 'x');
        end
        ref_point_cloud = ref_point_cloud + Points .* vis;
        sum_vis = sum_vis + vis;
    end
...
if (size(ref_point_cloud, 1) == 2)
    scatter(ref_point_cloud(1, :), ref_point_cloud(2, :), points_std_dev,  'co');
end
if (size(ref_point_cloud, 1) == 3)
    scatter3(ref_point_cloud(1, :), ref_point_cloud(2, :), ref_point_cloud(3, :), points_std_dev, 'co');
end
