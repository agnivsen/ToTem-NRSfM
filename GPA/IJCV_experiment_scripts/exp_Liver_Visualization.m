...
bestParams = {10, 100, 100, 0.01, 0.01, 0.01};
...
set(gca,'position',x1);
%hFig7 = tightfig(hFig7);
fig_saved_name = [fdir,  'exp_Landmark_visualization_', datasetCell{dchoice}, '_colorbar_uncroped'];
print(hFig7,  '-dpdf', fig_saved_name, '-painters');
% pdfcrop exp_Landmark_visualization_Liver_colorbar_uncroped.pdf
...
if dim == 3
    ...
    if (floor(fid/100) == 5)
        hold on;
        [~, TRI, ~] = ReadMeshFile ('../../dataset/Karim/KarimLiver/bending4.obj');
%        trimesh(TRI, refShape(1,:),refShape(2,:),refShape(3,:), codeVal, 'facecolor', 'interp');
        trimesh(TRI, refShape(1,:),refShape(2,:),refShape(3,:), codeVal);
...
%         set(gca,'position',x1);
        switch mod(fid, 100)
         ...
         title([figName, ':  CVE = ', num2str(cve, '%.2f')], 'horizontalAlignment', 'center', 'FontSize', 8);
         hold off;
    end
    if (floor(fid/100) == 6)
